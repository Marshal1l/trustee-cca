...
impl Verifier for CCA {
    async fn evaluate(
        ...
        debug!("expected_report_data: {:?}", expected_report_data);
...
fn assemble_cca_evidence(
    ...
    evidence.init_data = evidence.realm.cca_realm_personalization_value.clone();
    evidence.report_data = evidence.realm.cca_realm_challenge.clone();
